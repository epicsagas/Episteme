...
class CppParser(LanguageParser):
    ...
    # Regex patterns for C++ functions and methods
    FUNCTION_PATTERN = re.compile(
        r'(?:inline\s+)?(?:static\s+)?(?:virtual\s+)?(?:explicit\s+)?(?:constexpr\s+)?'
        r'(?:[\w:]+\s+)*?(\w+)\s*(?:const)?\s*(?:noexcept)?\s*\([^)]*\)',
        re.MULTILINE
    )
    ...
    def parse_code(self, code: str, file_name: str = "temp.cpp") -> List[SmellDetection]:
    ...
    def _parse_functions(self, code: str, file_name: str) -> List[SmellDetection]:
        """Parse and analyze functions and methods"""
        detections = []
    ...
    def _parse_classes(self, code: str, file_name: str) -> List[SmellDetection]:
        """Parse and analyze classes and structs"""
        detections = []
        ...
        for match in self.CLASS_PATTERN.finditer(code):
            ...
            field_count = self._count_class_fields(class_body)

            # Calculate metrics
            metrics = self._calculate_class_metrics(class_body, method_count, field_count)
            location = f"{file_name}:{line_no}"
    ...
    def _calculate_function_metrics(self, signature: str, body: str) -> CodeMetrics:
        ...
        if param_start != -1 and param_end != -1:
            ...
            if params_str:
                # Remove pointer/reference syntax
        ...
        local_vars = len(re.findall(r'\b(?:int|double|float|bool|string|char|void|auto|const)\s+\w+', body))
    ...
    def _calculate_class_metrics(self, class_body: str, method_count: int, field_count: int) -> CodeMetrics:
    ...
    def _count_class_methods(self, class_body: str) -> int:
        ...
        # Count function-like declarations with parentheses
        method_pattern = re.compile(r'\w+\s*\([^)]*\)\s*(?:const)?(?:override)?\s*[{;]')
        return len(method_pattern.findall(class_body))

    def _count_class_fields(self, class_body: str) -> int:
        """Count fields/members in class"""
        field_count = 0
        for line in class_body.split('\n'):
            line = line.strip()
            # Skip empty lines, braces, and comments
            if line and line not in ['{', '}'] and not line.startswith('//'):
                # Lines that look like field declarations (with type and semicolon)
                if re.match(r'(?:public|private|protected)?\s*(?:static\s+)?(?:const\s+)?[\w:]+\s+\w+\s*[=;]', line):
                    field_count += 1
        return field_count
    ...
    def _calculate_cc(self, code: str) -> int:
        ...
        cc += len(re.findall(r'\bdo\b', code))
