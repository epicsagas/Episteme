"""
Kotlin code smell parser using regex-based analysis
"""
...
class KotlinParser(LanguageParser):
    """Kotlin parser using regex patterns"""

    # Regex patterns for Kotlin functions and classes
    FUNCTION_PATTERN = re.compile(
        r'(?:public|private|protected|internal)?\s*(?:suspend)?\s*(?:inline)?\s*'
        r'fun\s+(?:<[^>]*>)?\s*(?:\w+\.)?(\w+)\s*\([^)]*\)(?:\s*:\s*[\w<>]+)?\s*\{',
        re.MULTILINE
    )

    CLASS_PATTERN = re.compile(
        r'(?:public|private|protected|internal)?\s*(?:abstract|sealed|data|enum)?\s*'
    ...
    def parse_file(self, file_path: str) -> List[SmellDetection]:
        """Parse a Kotlin file and detect code smells"""
        try:
    ...
    def parse_code(self, code: str, file_name: str = "temp.kt") -> List[SmellDetection]:
        """Parse Kotlin code string and detect code smells"""
        detections = []
    ...
    def _calculate_function_metrics(self, signature: str, body: str) -> CodeMetrics:
        ...
        if param_start != -1 and param_end != -1:
            ...
            if params_str:
                # Filter out 'vararg' and keep normal parameters
        ...
        local_vars = len(re.findall(r'\b(?:val|var)\s+\w+', body))
    ...
    def _count_class_methods(self, class_body: str) -> int:
        ...
        method_pattern = re.compile(
            r'(?:public|private|protected|internal)?\s*(?:abstract)?\s*(?:override)?\s*'
            r'fun\s+(?:<[^>]*>)?\s*\w+\s*\([^)]*\)\s*(?::\s*[\w<>]+)?\s*\{',
            re.MULTILINE
        )
    ...
    def _count_class_properties(self, class_body: str) -> int:
        ...
        for line in class_body.split('\n'):
            line = line.strip()
            # Properties: val name: Type or var name: Type
            if re.match(r'(?:public|private|protected|internal)?\s*(?:override)?\s*(?:val|var)\s+\w+\s*:', line):
                property_count += 1
    ...
    def _calculate_cc(self, code: str) -> int:
        ...
        cc += len(re.findall(r'\bis\b', code))  # 'is' branches in when
        ...
        # Kotlin-specific: lambda/block complexity
        cc += len(re.findall(r'\{.*?\}', code))  # Count lambda blocks
