# Alcove + Episteme Integration Guide

> Agent-first guide: combine generic software engineering knowledge (Episteme) with team-specific domain knowledge (Alcove) through MCP and natural-language workflows.

## Overview

**Episteme** provides universal knowledge (GoF patterns, refactorings, laws) as a read-only knowledge graph.  
...
- Apply generic best practices (Episteme)
- Respect team-specific constraints (Alcove)
- Cite both sources in recommendations

### Decision Priority

When Episteme and Alcove conflict, **Alcove wins** for final implementation guidance.
- **Episteme**: reference knowledge (general patterns/laws/smells)
...
3. Agent retrieves generic engineering guidance from Episteme.
...
For Cursor/Codex/other MCP-capable coding agents, register both MCP servers in each tool's MCP config and keep the same server names (`episteme`, `alcove`) so prompts and skills stay portable.

### 4. Document Linking Convention

Reference Episteme entities in Alcove docs:
...
- "Refactor this service following DR-112 and related Episteme laws"
...
Always link Alcove decisions to Episteme entities when applicable:
...
Don't duplicate Episteme content. Reference it:
...
When team conventions override Episteme advice:
...
├── DECISION.md        # ADRs with Episteme refs
...
## Advanced: Episteme → Alcove Feedback Loop

### Track Pattern Usage with Prometheus Metrics

Instrument your code to expose Episteme entity usage as Prometheus metrics:
...
### Issue: Episteme and Alcove Conflict

**Example**: Episteme says "Singleton OK", team doc says "Singleton banned"
...
1. Ask with intent + context target:
   - "Use Alcove docs + Episteme entities to review this code"
2. Request explicit dual citation:
   - "Cite DR/ARCH doc and Episteme ID in your answer"
...
**Decision Priority**: Alcove > Episteme (team rules override generic advice)
...
**Maintenance**: 
- Episteme: No action required (upstream handles updates)
