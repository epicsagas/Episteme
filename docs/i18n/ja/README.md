...
`epis install` はナレッジグラフデータベースを初期化し、ポート58302でHTTP APIサーバーを起動します。その後、新しいセッションを起動すれば完了です。エージェントやIDEの設定は変更しません。統合は以下の明示的なコマンドでのみ行われます（オプトイン）。
...
> **サーバー設定：** インストールウィザードがバインドアドレス（`127.0.0.1` はローカルのみ、`0.0.0.0` はネットワーク公開）を選択し、認証用ベアラートークンの生成可否を尋ねます。トークンのプロンプトはローカルでは既定で**いいえ**、`0.0.0.0` では既定で**はい**です。いずれも上書きできます。
...
Epistemeはローカル埋め込みによるセマンティック検索を提供します — 外部APIは不要です。

### デフォルト: MultilingualE5Small (同梱)

`epis install` は **MultilingualE5Small**（384次元、ONNX Runtime）で事前に埋め込み済みの913チャンクを含む**ビルド済みデータベース**を提供します。これにより:

- インストール後に**インデックス構築は不要** — 検索はすぐに使えます
- **完全オフライン** — fastembed（ONNX Runtime）でローカル実行
- **多言語対応** — 英語、韓国語、日本語、中国語など90以上の言語をサポート

### カスタムモデルを使用する

別のローカルモデルに切り替えるには、環境変数を設定してリビルドします:
...
利用可能なローカルモデル（ONNX、APIキー不要）:

| モデル | 次元数 | 向いているケース |
|--------|--------|-----------------|
| `MultilingualE5Small`（デフォルト） | 384 | 多言語、速度と品質のバランス |
| `AllMiniLML6V2` | 384 | 英語特化、高速 |
| `BGEBaseEN` | 768 | 英語、高品質 |

全モデル一覧は [対応埋め込みモデル](https://github.com/epicsagas/llm-kernel/blob/main/EMBEDDING_MODELS.md) を参照してください。
...
コンパイル時に `openai-embeddings` フィーチャーフラグが必要です。

> **注意:** モデルを切り替えた後は、必ず `epis build --rebuild` を実行して埋め込みを再生成してください。データベースは使用したモデルを記録しており、不一致がある場合は警告を表示します。
...
Episteme には知識を記録するための 2 つのレイヤーがあります。**カノニカルグラフ**（普遍的なパターン、法則、スメル）と**暗黙知レイヤー**（TK-* — カノニカルエンティティに自動リンクされるチームのインサイト）です。より充実したプロジェクトドキュメント — アーキテクチャ決定、コーディング規約、オンボーディングガイド、技術的負債の記録 — については、**[Alcove](https://github.com/epicsagas/alcove)** が推奨されるコンパニオンです。
...
### Episteme vs Alcove — どちらを使うべきか

| シナリオ | 使用するツール | 理由 |
|----------|-----|-----|
| モジュール内のコードスメルを検出する | **Episteme** `analyze_code` | 正規表現/AST による検出 + ランク付けされたリファクタリング提案 |
| 一時的なインサイトを記録する（「ここではいつも N+1 が発生する」） | **Episteme** `add_insight` | 関連するカノニカルエンティティ（SMELL-*, LAW-*）に自動リンク |
| SRP と Extract Class の関係を見つける | **Episteme** `find_path` | エンティティタイプを横断するマルチホップグラフ探索 |
| 新規プロジェクトのドキュメントを開始する | **Alcove** `init_project` | 7 つのコアテンプレート（PRD, ARCHITECTURE, DECISIONS, ...）が自動生成 |
| 正式なアーキテクチャ決定（ADR）を記録する | **Alcove** DECISIONS.md | コンテキスト、選択肢、結果を含む構造化 ADR フォーマット |
| ドキュメントが古くなっていないか、リンク切れがないか確認する | **Alcove** `lint_project` | WIP/TODO/DEPRECATED マーカー、孤立ファイル、古い日付を検出 |
| 命名規則や必須セクションを強制する | **Alcove** `validate_docs` | ポリシベースの検証（pass/warn/fail） |
| Obsidian メモをインポートしてエージェントからアクセス可能にする | **Alcove** `promote_document` | vault のシンボリックリンク + BM25/ベクトルインデックス |
| 原則とチームルールの両方に基づいた推奨を行う | **両方** | 普遍的な知識 + チーム固有の制約 |

### 暗黙知（TK-*）と Alcove ドキュメントの違い

Episteme の暗黙知レイヤーは、ナレッジグラフに自動接続される**短く一時的なインサイト**を対象としています — 「X の理由でポーリングではなくイベント駆動を選択した」というメモが、DP-018（Observer）と LAW-012（Fail Fast）に自動リンクされる、といった具合です。一方、Alcove は**構造化された長期ドキュメント**を扱います — セクションを持つ完全な ADR、アーキテクチャ図、コーディング標準、オンボーディングチェックリストなどです。

| | Episteme TK-* | Alcove |
|---|---|---|
| **粒度** | 原子的な自由テキストのインサイト | 構造化されたマルチセクションドキュメント |
| **自動リンク** | キーワード検出 → カノニカルエンティティ | ドキュメント間の wikilink |
| **ライフサイクル** | 作成 + 検索 | 完全な CRUD + 検証 + lint + 監査 + バックアップ |
| **検索** | FTS5 キーワード | BM25 + ベクトルハイブリッド（CJK 対応） |
| **用途** | 簡易な観察、教訓の記録 | 正式な決定記録、プロジェクトスキャフォールディング、ドキュメントガバナンス |

Alcove は 3 階層のドキュメント（7 コア + 19 補助 + 15 パブリックファイル）を管理し、CJK 対応の BM25 + ベクトルハイブリッド検索を提供し、Obsidian vault と連携します。ポリシー検証、セマンティックリント（リンク切れ、古いマーカー、孤立ファイル）、Git ベースのバックアップ機能も含まれています。

**詳細な分析**: [Alcove エコシステム比較](../../alcove-ecosystem.md) — ストレージモデル、検索機能、機能の完全性、ユースケースの意思決定マトリクス。

**利用パターン**: [Alcove 連携ガイド](../../alcove-integration.md) — エージェントワークフロー、デュアルコンテキストによるコードレビュー、セットアップ手順。
...
- [x] **クロスプラットフォームビルド** — macOS、Linux、Windows (DirectML GPU アクセラレーション対応)
