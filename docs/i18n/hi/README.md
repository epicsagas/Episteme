...
`epis install` नॉलेज ग्राफ डेटाबेस को इनिशियलाइज़ करता है और पोर्ट 58302 पर HTTP API सर्वर शुरू करता है। इसके बाद नया सेशन शुरू करें और तुरंत उपलब्ध है। यह एजेंट या IDE कॉन्फ़िगरेशन को नहीं बदलता; इंटीग्रेशन केवल नीचे दिए गए स्पष्ट कमांड से ऑप्ट-इन होते हैं।
...
> **सर्वर कॉन्फ़िगरेशन:** इंस्टॉल विज़ार्ड बाइंड एड्रेस (`127.0.0.1` केवल लोकल, `0.0.0.0` नेटवर्क के लिए) चुनने और प्रमाणीकरण के लिए बेयर टोकन जेनरेट करने को कहेगा। टोकन प्रॉम्प्ट का डिफ़ॉल्ट लोकल पर **नहीं** और `0.0.0.0` पर **हाँ** है; दोनों में इसे बदला जा सकता है।
...
Episteme सिमेंटिक सर्च के लिए स्थानीय एम्बेडिंग का उपयोग करता है — किसी बाहरी API की आवश्यकता नहीं।

### डिफ़ॉल्ट: MultilingualE5Small (शामिल है)

`epis install` एक **पूर्व-निर्मित डेटाबेस** के साथ आता है जिसमें **MultilingualE5Small** (384 आयाम, ONNX Runtime) का उपयोग करके 913 चंक्स पहले से एम्बेड किए गए हैं। इसका अर्थ है:

- **इंस्टॉल के बाद इंडेक्सिंग की ज़रूरत नहीं** — सर्च तुरंत काम करती है
- **पूरी तरह ऑफलाइन** — मॉडल fastembed (ONNX Runtime) के ज़रिए स्थानीय रूप से चलता है
- **बहुभाषी** — अंग्रेज़ी, कोरियाई, जापानी, चीनी और 90+ भाषाओं का समर्थन करता है

### कस्टम मॉडल का उपयोग करना

किसी भिन्न स्थानीय मॉडल पर जाने के लिए, पर्यावरण चर सेट करें और पुनः निर्माण करें:

```bash
# अपना पसंदीदा मॉडल सेट करें
export EPISTEME_EMBEDDING_MODEL=AllMiniLML6V2

# नए मॉडल के साथ इंडेक्स पुनः बनाएँ
epis build --rebuild
```

उपलब्ध स्थानीय मॉडल (ONNX, API कुंजी की ज़रूरत नहीं):

| मॉडल | आयाम | सबसे उपयुक्त |
|------|------|--------------|
| `MultilingualE5Small` (डिफ़ॉल्ट) | 384 | बहुभाषी, गति और गुणवत्ता में संतुलन |
| `AllMiniLML6V2` | 384 | अंग्रेज़ी-केंद्रित, तेज़ |
| `BGEBaseEN` | 768 | अंग्रेज़ी, उच्च गुणवत्ता |

पूर्ण सूची के लिए [समर्थित एम्बेडिंग मॉडल](https://github.com/epicsagas/llm-kernel/blob/main/EMBEDDING_MODELS.md) देखें।

### OpenAI एम्बेडिंग का उपयोग करना

```bash
# OpenAI प्रोवाइडर सक्षम करें
...
# OpenAI के साथ पुनः निर्माण करें
epis build --rebuild
```

संकलन समय पर `openai-embeddings` फ़ीचर फ़्लैग की आवश्यकता है।

> **नोट:** मॉडल बदलने के बाद, एम्बेडिंग पुनः उत्पन्न करने के लिए **अवश्य** `epis build --rebuild` चलाएँ। डेटाबेस यह दर्ज करता है कि कौन सा मॉडल उपयोग किया गया था और बेमेल होने पर चेतावनी देगा।
...
## पारिस्थितिकी तंत्र: Alcove एकीकरण

Episteme में ज्ञान कैप्चर करने के लिए दो परतें हैं: **विहित ग्राफ़** (सार्वभौमिक पैटर्न, नियम, स्मेल) और **अव्यक्त ज्ञान परत** (TK-* — टीम की अंतर्दृष्टियाँ जो स्वतः विहित एंटिटीज़ से जुड़ जाती हैं)। समृद्ध प्रोजेक्ट प्रलेखन के लिए — आर्किटेक्चर निर्णय, कोडिंग सम्मेलन, ऑनबोर्डिंग गाइड, तकनीकी ऋण रजिस्टर — **[Alcove](https://github.com/epicsagas/alcove)** अनुशंसित साथी है।
...
### Episteme बनाम Alcove — कब किसका उपयोग करें

| परिदृश्य | उपयोग करें | कारण |
|----------|-----|-----|
| किसी मॉड्यूल में कोड स्मेल का पता लगाएँ | **Episteme** `analyze_code` | Regex/AST पहचान + क्रमबद्ध रिफैक्टरिंग सुझाव |
| एक क्षणिक अंतर्दृष्टि दर्ज करें ("हमें यहाँ हमेशा N+1 का सामना करना पड़ता है") | **Episteme** `add_insight` | प्रासंगिक विहित एंटिटीज़ (SMELL-*, LAW-*) से स्वतः लिंक |
| SRP और Extract Class के बीच संबंध खोजें | **Episteme** `find_path` | एंटिटी प्रकारों में बहु-हॉप ग्राफ़ ट्रैवर्सल |
| नए प्रोजेक्ट के लिए प्रलेखन शुरू करें | **Alcove** `init_project` | 7 मुख्य टेम्पलेट (PRD, ARCHITECTURE, DECISIONS, ...) स्वतः उत्पन्न |
| एक औपचारिक आर्किटेक्चर निर्णय (ADR) दर्ज करें | **Alcove** DECISIONS.md | संदर्भ, विकल्प, परिणामों के साथ संरचित ADR प्रारूप |
| जाँचें कि प्रलेखन पुराना है या टूटे हुए लिंक हैं | **Alcove** `lint_project` | WIP/TODO/DEPRECATED मार्कर, अनाथ फ़ाइलें, पुरानी तिथियाँ का पता लगाता है |
| नामकरण सम्मेलनों या आवश्यक अनुभागों को लागू करें | **Alcove** `validate_docs` | पास/चेतावनी/विफल के साथ नीति-आधारित सत्यापन |
| एजेंट पहुँच के लिए Obsidian नोट्स आयात करें | **Alcove** `promote_document` | सिमलिंक वॉल्ट + BM25/वेक्टर इंडेक्सिंग |
| सिफारिश को सिद्धांतों और टीम नियमों दोनों में आधारित करें | **दोनों** | सार्वभौमिक ज्ञान + टीम-विशिष्ट बाधाएँ |

### अव्यक्त ज्ञान (TK-*) बनाम Alcove प्रलेखन

Episteme की अव्यक्त ज्ञान परत **लघु, क्षणिक अंतर्दृष्टियों** के लिए डिज़ाइन की गई है जो स्वतः ज्ञान ग्राफ़ से जुड़ती हैं — "हमने पोलिंग के बजाय इवेंट-संचालित चुना क्योंकि X", स्वतः DP-018 (Observer) और LAW-012 (Fail Fast) से लिंक हो गया। Alcove **संरचित, दीर्घकालिक प्रलेखन** को संभालता है — अनुभागों वाले पूर्ण ADR, आर्किटेक्चर आरेख, कोडिंग मानक, ऑनबोर्डिंग चेकलिस्ट।

| | Episteme TK-* | Alcove |
|---|---|---|
| **ग्रैन्युलैरिटी** | परमाणु मुक्त-पाठ अंतर्दृष्टि | संरचित बहु-अनुभाग दस्तावेज़ |
| **स्वतः लिंकिंग** | कीवर्ड पहचान → विहित एंटिटीज़ | दस्तावेज़ों के बीच विकिलिंक |
| **जीवनचक्र** | बनाएँ + खोजें | पूर्ण CRUD + सत्यापन + लिंट + ऑडिट + बैकअप |
| **खोज** | FTS5 कीवर्ड | BM25 + वेक्टर हाइब्रिड (CJK समर्थन) |
| **सबसे उपयुक्त** | त्वरित अवलोकन, सीखे गए पाठ | औपचारिक निर्णय, प्रोजेक्ट स्कैफोल्डिंग, प्रलेखन अनुशासन |

Alcove प्रलेखन के 3 स्तरों का प्रबंधन करता है (7 मुख्य + 19 सहायक + 15 सार्वजनिक फ़ाइलें), CJK समर्थन के साथ BM25 + वेक्टर हाइब्रिड खोज प्रदान करता है, और Obsidian वॉल्ट के साथ एकीकृत होता है। इसमें नीति सत्यापन, सिमैंटिक लिंटिंग (टूटे हुए लिंक, पुराने मार्कर, अनाथ), और git-आधारित बैकअप शामिल हैं।

**पूर्ण विश्लेषण**: [Alcove पारिस्थितिकी तंत्र तुलना](../../alcove-ecosystem.md) — भंडारण मॉडल, खोज क्षमताएँ, सुविधा पूर्णता, और उपयोग-मामला निर्णय मैट्रिक्स।

**उपयोग पैटर्न**: [Alcove एकीकरण गाइड](../../alcove-integration.md) — एजेंट वर्कफ़्लो, दोहरे संदर्भ के साथ कोड समीक्षा, और सेटअप निर्देश।
