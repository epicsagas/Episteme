...
<p align="center">一个离线优先、单二进制文件的知识图谱，通过语义关系将设计模式、重构技术和软件法则连接在一起。<br><b>优先为 AI 智能体而生</b> —— 以五主机插件（Claude Code · Codex · Grok Build · agy · Hermes）形式提供，并可集成到 Cursor 及其他兼容 MCP 的工具中。</p>
...
agy 与 Hermes 没有安装钩子，因此请先安装二进制文件（参见[手动安装](#手动安装)），然后添加插件：
...
> Hermes 的安装扫描器可能会将本仓库的文件（`AGENTS.md`、`Cargo.toml`、docs 等）误判为 CRITICAL 持久化并阻止安装。可以传入 `--force`，或在 Hermes 配置中设置 `plugins.scan_on_install: false` 关闭扫描。

### 安装后（Claude Code、Codex、Grok Build）
...
| 👃 | **23种代码异味类型** | Long Method、God Object、Feature Envy等 ¹ |
...
| 🌐 | **REST API（20+个端点）** | 认证、速率限制、健康探针、Prometheus指标 |
...
Episteme 使用本地嵌入进行语义搜索 —— 无需外部 API。

### 默认：MultilingualE5Small（已内置）

`epis install` 附带一个**预构建数据库**，其中已使用 **MultilingualE5Small**（384 维，ONNX Runtime）对 913 个文本块完成了嵌入。这意味着：

- **安装后无需索引** —— 搜索立即可用
- **完全离线** —— 模型通过 fastembed (ONNX Runtime) 在本地运行
- **多语言支持** —— 支持英语、韩语、日语、中文及 90 多种语言

### 使用自定义模型

如需切换到其他本地模型，请设置环境变量后重新构建：
...
可用的本地模型（ONNX，无需 API 密钥）：

| 模型 | 维度 | 适用场景 |
|------|------|---------|
| `MultilingualE5Small`（默认） | 384 | 多语言，速度与质量均衡 |
| `AllMiniLML6V2` | 384 | 英语优化，速度快 |
| `BGEBaseEN` | 768 | 英语，质量更高 |

完整目录请参阅[支持的嵌入模型](https://github.com/epicsagas/llm-kernel/blob/main/EMBEDDING_MODELS.md)。
...
需要在编译时启用 `openai-embeddings` feature flag。

> **注意：** 切换模型后，**必须**运行 `epis build --rebuild` 以重新生成嵌入。数据库会记录所使用的模型，若出现不匹配将给出警告。
...
> 更多帮助：[故障排除部分](../../README.md#troubleshooting) · [提交 Issue](https://github.com/epicsagas/Episteme/issues)
